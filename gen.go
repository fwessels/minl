...
type LambdaMetadata struct {
	PackageName string   // Name for your lambda func.
	Endpoint    string   // Endpoint server when the listen bucket notification API would be inititated.
	AccessKey   string   // Access key.
	SecretKey   string   // Secret key.
	Secure      bool     // Secure set when environment variable is set.
	Region      string   // Region set for minio lambda resource names.
	Bucket      string   // Bucket set for notification.
	Events      []string // Collection of events.
	Prefix      string   // Prefix set for notification to listen on.
	Suffix      string   // Suffix set for notification to listen on.
}
...
func checkGenSyntax(ctx *cli.Context) {
	if !ctx.Args().Present() {
		cli.ShowCommandHelpAndExit(ctx, "gen", 1)
	}
}
...
func mainGen(ctx *cli.Context) {
	...
	tmpl := template.Must(template.New(lambda).Parse(lambdaFunc))
